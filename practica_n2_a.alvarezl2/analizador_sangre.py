...
def confirmar_anemia(examen:dict) -> bool:
    ...
    if examen["edad"] >= 12 and examen["edad"] <=14  and hto >= 36:
        return True
    if examen["edad"] >= 15 and examen["genero"] == "M" and confirmar_embarazo(examen) == False and hto >= 36:
        return True
    if examen["edad"] >= 15 and examen["genero"] == "F" and confirmar_embarazo(examen) == True and hto >= 33:
        return True
    if examen["edad"] >= 15 and examen["genero"] == "M" and hto >= 39:
        return True
...
def calcular_riesgo_cardiaco(examen:dict)->int:
    ...
    if examen["genero"] == "M" and examen["HDL"] < 40:
        resultado += 1
    elif examen["genero"] == "F" and examen["HDL"] < 50:
        resultado += 1
...
def clasificar_por_estado_diabetes(e1:dict, e2:dict, e3:dict, e4:dict) -> dict:
    ...
    examenes = [e1, e2, e3, e4]

    for e in examenes:
        y = validar_diabetes(e)
        if y == "El paciente presenta diabetes":
            i += 1
        if y == "El paciente presenta prediabetes":
            u += 1
        if y == "El paciente no presenta ninguna anormalidad":
            a += 1
    
    return x

def genero(examen: dict) -> int:
...
def analizar_datos_embarazosos(e1:dict, e2:dict, e3:dict, e4:dict) -> str:
    ...
    return str("De los 4 exámenes recibidos: ") + str(HH) + str(" son hombres y ") + str(FF) + str(" son mujeres.") + str("\n") + str("de las mujeres:") + str(TT) + str(" están embarazadas y FF no lo están")